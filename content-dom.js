...
async function fetchImageBlob(imgSrc, filename = "image.png") {
  ...
  // Screenshots generated by captureVisibleTab are already local data URLs.
  // Convert them directly instead of sending them through the network-only
...
function _expandedXAnchorUrls(anchor, label = "") {
  if (SITE !== "x" || !anchor) return [];
  const values = [
...
function _collectPostOutboundLinks(element, summaryText = "") {
  ...
  for (const container of containers) {
    ...
    for (const anchor of container.querySelectorAll("a[href]")) {
      ...
      // X hides outbound destinations behind t.co. Its rendered anchor keeps
      // the expanded GitHub/GitLab URL in attributes or display text.
      for (const expandedUrl of _expandedXAnchorUrls(anchor, label)) {
        const expanded = _classifyRelatedUrl(expandedUrl, label, "x-expanded-link");
        if (expanded) links.push(expanded);
      }
...
window.fbsExpandedXAnchorUrls = _expandedXAnchorUrls;
