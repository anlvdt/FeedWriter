 ...
 *   affiliate — boost product/review/deal signals; still hard-penalize pure spam hard-sell
 *
 * CommonJS for node:test via createRequire.
 */
"use strict";

const VALID_PROFILES = ["tech", "general", "affiliate"];
...
// Affiliate-positive: product/review/deal content worth summarizing
const AFFILIATE_POSITIVE = [
  "review",
  "đánh giá",
  "trải nghiệm",
  "so sánh",
...
/**
 * @param {string} text
 * @param {'tech'|'general'|'affiliate'} [profile='tech']
...
function scoreText(text, profile = "tech") {
  ...
  // Baseline: tech=3 (strict), general=4 (more balanced), affiliate=3.5
  ...
  // --- Affiliate positive boosts ---
  if (p === "affiliate") {
    // Product/review signals: strong boost for ranking affiliate-worthy content
    score += sumBoost(lower, AFFILIATE_POSITIVE, 1.5, 5);
  } else if (p === "general") {
    // Mild interest in useful product content
    score += sumBoost(lower, AFFILIATE_POSITIVE, 0.5, 1.5);
  }

  // --- Spam hard-sell (all profiles, hard penalty) ---
  // Pure spam like "mua ngay flash sale shopee" should stay low everywhere.
  // On affiliate, product keywords above may lift review content, but
  ...
  // affiliate: keep drama penalty, lighter lifestyle
  let offTopicHits = 0;
